...
<script type="text/javascript">
	...
	function go_search() {
		...
		theForm.action = "/admin/order/cancelList";
		theForm.submit();
	}
	...
	function go_order_save() {
		...
		if (count == 0) {
			alert("반품처리할 항목을 선택해 주세요.");
		} else {
			document.frm.action = "/admin/order_cancel";
			document.frm.submit();
		}
	}
...
<body>
	...
	<div id="table" align="center">
			...
			<table class="list">
				<tr>
					<td colspan="10" style="border: gray; text-align: right">
						<h2 style="text-align: center; margin-bottom:25px">반품리스트</h2>
					</td>
				</tr>
				<tr>
					...
					<th>전화</th>
					<th>주문일</th>
					<th><input type="button" value="반품처리(상품확인)"
						onClick="go_order_save()"> <input
				...
				<c:forEach var="orderList" items="${orderList}">
					<tr class="record">
						<td><c:choose>
								<c:when test='${orderList.result=="3"}'>
		...
		<form id='actionForm' action="/admin/order/cancelList" method='get'>
			<input type='hidden' name='pageNum' value='${pageMaker.cri2.pageNum}'>
			<input type='hidden' name='amount' value='${pageMaker.cri2.amount}'>
			<input type='hidden' name='keyword' value='${pageMaker.cri2.keyword}'>
			<input type='hidden' name='type' value='${pageMaker.cri2.type}'>
		...
		<div id="main_footer">
			...
			<form id='searchForm' action="/admin/order/cancelList" method='get'>
