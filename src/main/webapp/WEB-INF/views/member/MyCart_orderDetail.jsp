			...
			<c:otherwise>
				...
				<c:forEach var="orderList" items="${orderList}">
						<div class="current_item_list">
							<ul>
								<li><a href='/product/detail?prodno=<c:out value="${orderList.prodno}"/>'>
								...
								<li>
									<table id="pay">
										<tr>
											<td><fmt:formatNumber type="number" maxFractionDigits="3" value="${orderList.price}" />원</td>
										</tr>
										<tr>
											<td><c:out value="${orderList.num}"/>개</td>
										</tr>
									</table>
								</li>
								<li>
									<table id="money">
										<tr>
											<td><fmt:formatNumber type="number" maxFractionDigits="3" value="${orderList.price * orderList.num}" />원
											</td>
				...
				<c:choose>
					<c:when test='${orderList[0].result=="1"}'> 
						<div id="buttons" style="float: right;">
							<input type="button" value="주문 취소하기" class="cancel" id="delete" 
							       onclick="location.href='/order/delete?orderno=<c:out value='${orderList[0].orderno}'/>'">
						</div>
					</c:when>
					<c:otherwise>
						<div id="buttons" style="float: right;">
							<input type="button" value="반품 신청하기" class="cancel" id="cancel" 
								   onclick="location.href='/order/cancel?orderno=<c:out value='${orderList[0].orderno}'/>'">
							<input type="button" value="리뷰쓰러가기" class="review" id="review"
								   onclick="location.href='/product/detail?prodno=<c:out value='${orderList[0].prodno}'/>#box2'">
						</div>
					</c:otherwise>
				</c:choose>
			</c:otherwise>
...
<input type="hidden" name="orderno" value='<c:out value="${orderList[0].orderno}"/>' >
